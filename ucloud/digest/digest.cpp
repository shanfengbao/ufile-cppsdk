...
UFileDigest::UFileDigest(const std::string &publicKey,
                         const std::string &privateKey)
    : m_ak(publicKey), m_sk(privateKey) {}

UFileDigest::~UFileDigest() {}

void UFileDigest::SetKey(const std::string &publicKey,
                         const std::string &privateKey) {
...
// return: signature
int UFileDigest::SignWithData(const std::string &utf8String2Sign,
  ...
  HMAC_SHA1(digest, (unsigned char *)m_sk.c_str(), m_sk.size(),
            (unsigned char *)utf8String2Sign.c_str(), utf8String2Sign.size());
  signature = base64_encode(digest, 20, BASE64_STD);
  return 0;
}

int UFileDigest::SignWithRequest(
...
std::string UFileDigest::CanonicalizedResource(const std::string &bucket,
                                               const std::string &key) {
...
std::string UFileDigest::CanonicalizedUCloudHeaders(HTTPRequest *http) {
  ...
  for (; it != headers.end(); ++it) {
    ...
    if (memcmp(k.c_str(), "x-ucloud-", 9) != 0)
      continue;
